...
import { DriverType, MergeWithOverwrite } from '../types/types';
...
interface Props {
  onNewDriver?: (driver: DriverType) => void;
}
  ...
  function createDriver(onSuccess?: () => void) {
    ...
    toast.promise(uploader, {
      ...
      success: (c) => {
        if (onSuccess) {
          onSuccess();
          onNewDriver?.(driverWithId as DriverType);
        } else navigate({to: `/drivers/${slug}/edit`});
